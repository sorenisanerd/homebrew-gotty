...
class Gotty < Formula
  ...
  if OS.mac? && Hardware::CPU.arm?
      url "https://github.com/sorenisanerd/gotty/releases/download/${TAG_NAME}/gotty_${TAG_NAME}_darwin_arm64.tar.gz"
      sha256 "$SHA256_DARWIN_ARM"
  end

  if OS.mac? && Hardware::CPU.intel?
      url "https://github.com/sorenisanerd/gotty/releases/download/${TAG_NAME}/gotty_${TAG_NAME}_darwin_amd64.tar.gz"
      sha256 "$SHA256_DARWIN"
  end

  if OS.linux? && Hardware::CPU.is_64_bit?
      url "https://github.com/sorenisanerd/gotty/releases/download/${TAG_NAME}/gotty_${TAG_NAME}_linux_amd64.tar.gz"
      sha256 "$SHA256_LINUX"
  end
